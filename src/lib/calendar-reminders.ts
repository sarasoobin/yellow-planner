...
import { REPEAT_FREQUENCIES, type Item, type ItemStyle } from '@/lib/types'
...
export const calendarChangeSchema = z.discriminatedUnion('type', [
  ...
  z.object({ type: z.literal('text'), id: idSchema, text: z.string().max(5000), afterId: idSchema.optional(), kind: taskKindSchema.optional() }),
...
export function applyCalendarChange(note: CalendarNote, change: CalendarChange): CalendarNote {
  ...
  if (change.type === 'text' && !note.tasks.some((task) => task.id === change.id)) {
    ...
    const after = tasks.findIndex((task) => task.id === change.afterId)
    tasks.splice(after < 0 ? tasks.length : after + 1, 0, { ...blankCalendarTask(change.id, change.kind ?? 'todo'), text: change.text })
    return { ...note, tasks }
  }
  return {
    ...note,
