...
- **한 칸을 통째로 적는 방식으로 바꿈.** 줄 목록이 아니라 종이 한 면처럼
  마구잡이로 쓰고, Enter로 줄을 바꾸고, 칸 끝에 닿으면 다음 줄로 이어진다
- 체크박스는 글자(☐ / ☑)로 넣는다. 줄 앞이든 문장 중간이든 아무 자리에나
  그릴 수 있고, 눌러서 체크한다
- 로그인·랜딩 화면을 노트 테마로 정리
- 완료 표시를 빨간 색연필 체크와 빨간 취소선으로. 흐려지는 효과는 종이에 없어서 뺌
- 이 달 메모는 체크리스트 대신 자유롭게 적는 칸으로

- 가입 없이 둘러보는 데모 계정 (`supabase/seed-demo.sql`)
- 표지에 월별 달성률 — 막대를 누르면 그 달로 이동
- 제목·로고·연도에 손글씨 폰트 (나눔손글씨 펜)
- **고른 글자만** 굵게·기울임·색·형광펜·크기 지정

### 보안
- 적은 글을 HTML로 저장하게 되면서, 저장 직전에 허용 목록에 없는 태그와
  style을 전부 걸러냅니다. 붙여넣기는 글자만 받습니다
- **RLS 교차 검증 완료.** 계정 A로 채운 표지·달력·주간·메모가 계정 B에서
  모두 비어 있고, A로 돌아오면 그대로 남아 있는 것을 확인했습니다
